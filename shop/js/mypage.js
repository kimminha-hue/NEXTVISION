...
function renderHeaderMenu() {
    ...
    if (isLoggedIn && (user.userIdx || user.id || user.loginId)) {
        navLinks.innerHTML = `
...
if (voiceSelect) voiceSelect.value = loginUser.voiceId || "voice_05";
...
if (profileForm) {
    profileForm.addEventListener("submit", async (e) => {
        ...
        const voiceId = voiceSelect ? voiceSelect.value : "voice_05";
...
if (savePasswordBtn) {
    savePasswordBtn.addEventListener("click", async () => {
        ...
        const newPasswordConfirm = confirmPasswordInput ? confirmPasswordInput.value.trim() : "";
...
function renderOrders(filter = "all") {
    ...
    const filteredOrders = userData.orders.filter((o) => {
        ...
        return o.status === filter || o.status === normalized[filter];
    });
